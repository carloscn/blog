...
学习目标和顺序：
* 【操作系统概念】：了解机制的发展史、原因；定义专业术语；理解当前设计的痛点和难点，优缺点（读论文）；更高一层级的去看下面的设计。
* 【ARM架构级支持】：了解底层实现原理和接口；区分操作系统和CPU的各自职责；
* 【操作系统实例】：以Linux或者RTOS主流的操作系统作为实例，了解如何对这些机制进行实现；对上对操作系统概念抽象的具体化，对下对接ARM架构；
